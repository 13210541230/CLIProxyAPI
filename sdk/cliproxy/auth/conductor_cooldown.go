...
// clearPersistedWrappedUpstreamRateLimitState removes cooldown state written by
// older versions before wrapped upstream 429s were classified as transient.
func clearPersistedWrappedUpstreamRateLimitState(auth *Auth, now time.Time) bool {
	if auth == nil {
		return false
	}
	changed := false
	for _, state := range auth.ModelStates {
		if state != nil && isWrappedUpstreamRateLimitResultError(state.LastError) {
			resetModelState(state, now)
			changed = true
		}
	}
	if isWrappedUpstreamRateLimitResultError(auth.LastError) {
		auth.LastError = nil
		auth.StatusMessage = ""
		auth.Unavailable = false
		auth.NextRetryAfter = time.Time{}
...
// isWrappedUpstreamRateLimitResultError identifies a 429 generated by an
// intermediary after the real upstream returned a rate limit. A plain provider
// 429 remains quota evidence and must keep the normal cooldown behavior.
func isWrappedUpstreamRateLimitResultError(err *Error) bool {
	if err == nil || statusCodeFromResult(err) != http.StatusTooManyRequests {
		return false
	}
	if strings.EqualFold(strings.TrimSpace(err.Code), "upstream_error") {
		return true
	}
	...
	if errDecode := json.Unmarshal([]byte(message), &payload); errDecode != nil {
		return false
	}
	if !strings.EqualFold(strings.TrimSpace(payload.Type), "upstream_error") &&
		!strings.EqualFold(strings.TrimSpace(payload.Code), "upstream_error") {
		return false
	}
	return payload.UpstreamStatus == 0 || payload.UpstreamStatus == http.StatusTooManyRequests
}

// isTransientUpstreamResultError reports known gateway-level upstream failures
// and stream interruptions. These mean the upstream side was temporarily
// unavailable (bad gateway, overload, or the response stream dropped mid-flight)
// rather than that the credential is invalid. Plain provider 429s and generic
// provider 503s retain their normal cooldown behavior.
func isTransientUpstreamResultError(err *Error) bool {
	...
	case http.StatusTooManyRequests:
		return isWrappedUpstreamRateLimitResultError(err)
	case http.StatusBadGateway, http.StatusGatewayTimeout:
		return true
	case http.StatusServiceUnavailable:
		return isTransientUpstreamServiceError(err.Message)
...
func isTransientUpstreamServiceError(message string) bool {
	m := strings.ToLower(strings.TrimSpace(message))
	return strings.Contains(m, "server_is_overloaded") ||
		strings.Contains(m, "incomplete sse") ||
		strings.Contains(m, "stream disconnected") ||
		strings.Contains(m, "upstream_error")
}
...
func isTransientUpstreamError(err error) bool {
	...
	case http.StatusBadGateway, http.StatusGatewayTimeout:
		return true
	case http.StatusServiceUnavailable:
		return isTransientUpstreamServiceError(err.Error())
	case http.StatusInternalServerError:
