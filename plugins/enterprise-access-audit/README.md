...
| `account_pool.max_busy_rejections` | `3` | Consecutive `account_busy` rejections (range 1-100) before the session fails over to another in-pool account. Any successful admission resets the streak, so transient overload never moves a session while a persistently unavailable account cannot strand its users. |
