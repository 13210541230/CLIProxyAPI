...
| `account_pool.enabled` | `false` | Master switch for account-pool scheduling and per-account concurrency admission. When disabled, stored pool policy and concurrency limits are not enforced; if `exclusive-scheduler-providers: [codex]` remains configured, the plugin delegates selection to CPA's built-in round-robin scheduler rather than selecting globally itself. When enabled, pool policy and explicit per-account limits apply. |
...
| `account_pool.max_busy_rejections` | `3` | Consecutive `account_busy` rejections (range 1-100) before the pool layer defers a bound session to the configured api-key provider (single-account realism: a pool session is **never** rebound to a sibling OAuth account). Any successful admission resets the streak. If no api-key provider is configured the session keeps receiving retryable `account_busy` on its own account until it drains. |
| `account_pool.session_idle_ttl_seconds` | `7200` | Idle gap after which a session binding expires and fresh account selection is allowed. Every request from the session (including ones served by the deferred api-key layer) refreshes the clock, so an active conversation never changes accounts — only a session silent for longer than this may rebind. This is also the sole rebind trigger after a pool policy publication or after the bound account cools down or is removed. |
| `exclusive-scheduler-providers` | — | Host-level sibling key `exclusive-scheduler-providers: [codex]` locks Codex scheduling to this plugin. The account-pool workspace toggle maintains `account_pool.enabled` and this claim together, so operators do not need to edit YAML manually. |
...
The resource is a self-contained HTML workspace for audit records, policy rows, plugin settings, and account-pool scheduling activation. The account-pool page shows separate policy and live scheduler-takeover status, and its activation toggle maintains both the account-pool switch and the Codex exclusive claim without manual YAML edits. It uses the Management Center's same-origin `postMessage` bridge for authenticated management calls, so the iframe never receives the Management API bearer key. The resource only displays masked Key hashes and must not call any endpoint that returns raw API Keys. For the user-facing workflow, audit filtering accepts a username, email, or department keyword through a pending filter draft; typing does not reload records until the operator clicks Apply filters. The user field provides candidate suggestions formatted as username, department, email, and Key hash suffix. Selecting a candidate applies the exact 8-character Key hash, which distinguishes duplicate usernames; free-text matching can still query all matching Keys. Clicking outside the input field or pressing Escape dismisses the candidate suggestions. Policy rows are grouped by user, including all hashes owned by that user, and multiple users can be selected for one batch policy update. The access-policy toolbar also provides a searchable multi-select department filter with select-all-visible and clear actions, using configured department names and falling back to IDs only when a name is unavailable. The model picker is populated from the current CPA model catalog and loaded auth-file model lists, while a manual model ID remains available for dynamic models. If the plugin is disabled or the resource is unavailable, the generic host removes the menu without affecting Enterprise Keys, Quota, or other core pages.
