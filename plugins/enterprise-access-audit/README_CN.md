...
插件资源页面中的“请求审计”在输入用户名、邮箱或部门时只更新待应用条件，不会因为输入变化而立即重新加载记录；点击“应用筛选”后才查询。输入框会提供候选建议，候选项按“用户名 · 部门 · 邮箱 · Key 尾码”展示。选择候选项后，查询只使用该 Key 的 8 位 Hash；如果直接输入普通关键字，则匹配该关键字对应的全部 Key。元数据暂时不可用时，应先恢复 `/v0/management/enterprise/key-bindings/metadata` 的管理访问权限。
