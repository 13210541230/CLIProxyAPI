...
通过 Management Center 的插件资源页面打开时，插件会读取企业 Key 的**非敏感元数据投影**，将审计记录和访问策略优先显示为用户名。该投影只包含 API Key Hash、用户名、邮箱、部门 ID 和已配置的部门名称，不包含原始 API Key。

- 审计和策略的内部关联仍使用 CPA 的 8 位 Key Hash，这是插件执行请求策略所需的标识；
- 页面主标识显示企业 Key 的用户名，同一用户名拥有多个 Key 时，候选项会同时显示部门、邮箱和 Key 尾码，从而可以选择单个 Key；
...
插件资源页面中的“请求审计”在输入用户名、邮箱或部门时只更新待应用条件，不会因为输入变化而立即重新加载记录；点击“应用筛选”后才查询。输入框会提供候选建议，候选项按“用户名 · 部门 · 邮箱 · Key 尾码”展示。选择候选项后，查询只使用该 Key 的 8 位 Hash；如果直接输入普通关键字，则匹配该关键字对应的全部 Key。点击输入框外部或按下 Esc 会收起候选建议。元数据暂时不可用时，应先恢复 `/v0/management/enterprise/key-bindings/metadata` 的管理访问权限。

## 7. Enterprise Key 禁止模型配置

模型目录保持完整可见，插件只在请求执行时校验策略。操作员可以在插件资源页面按用户名、邮箱或部门名称搜索用户，也可以打开部门多选筛选器，任意勾选多个部门后批量选择用户，为单个用户或多个用户设置禁止模型。列表中的部门展示使用 Enterprise Key 管理中配置的部门名称，找不到名称时才回退到部门 ID。
